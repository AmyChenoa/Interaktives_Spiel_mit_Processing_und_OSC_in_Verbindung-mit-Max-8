class Game {
  ...
  int highScore;
  boolean gameOver = false; // Neue Variable hinzufügen
  ...
  void startCampaign() {
    if (screen == 5) {
      println("Spiel wurde bereits gewonnen! Kampagne kann nicht neu gestartet werden.");
      sound.stopSound();
      sound.sendSound("Win");
      sound.sendSound("ambient"); // Testweise auskommentieren
      return;
    }
  ...
  void levelCompleted() {
    println("levelCompleted() aufgerufen! Kampagne: " + campaignMode + ", Level: " + currentLevel);

    if (campaignMode) {
      if (currentLevel < 9) {
        currentLevel++;
        println("Wechsle zu Level " + currentLevel);
        loadLevel(currentLevel);
      } else {
        println("Kampagne gewonnen! Win-Screen wird angezeigt.");
        screen = 5;
        gameOver = true; // Spiel wird beendet
      }
    } else {
      println("Kein Kampagnenmodus! Wechsel direkt zum Win-Screen.");
      screen = 5;
      sound.stopSound();
      sound.sendSound("Win");
      sound.sendSound("ambient"); // Testweise auskommentieren
    ...
    println("Nach levelCompleted(): screen = " + screen);
  }
  ...
  void triggerTransition(int newScreen) {
    ...
    // Stoppe Ambient-Sound und starte Game-Over-Sound, wenn das Spiel vorbei ist
    if (newScreen == 4) {
      sound.stopSound();
      sound.sendSound("gameover");
      sound.sendSound("ambient");
    }
  ...
  transitionProgress = lerp(transitionProgress, 1, 0.2); // oder höher, z.B. 0.3 für einen noch schnelleren Übergang
  ...
  void playGame() {
    ...
    if (gameOver) return; // Falls das Spiel vorbei ist, nichts mehr ausführen
  ...
  void drawHUD() {
    ...
    text("Points: " + player.score, 145, 20); // Zentrierter Punktetext
